...
nc.data.write <- function(file, raster_obj){
  ...
  for(n in 1:nlayers(raster_obj)){
    r <- raster_obj[[n]]
    nc_var <- ncvar_def(name = names(r)[1],dim = list(dim_x,dim_y), units = "", missval = NaN, prec ='integer',compression = 1)
    nc_var_list[[as.character(n)]] <- nc_var
  }

  nc <- nc_create(filename, nc_var_list)
  #nc_close(nc)
